...
parser.add_argument('--model', default='PRN', type=str, choices=['WRN'])
...
parser.add_argument('--out-dir', default='faal_wrn_out', type=str, help='Output directory')
...
if args.pre_trained == 'MART':
    ...
    model.load_state_dict(torch.load('WRN/models/cifar10/rob_cifar_mart.pt'))
elif args.pre_trained == 'AWP':
    from robustbench.model_zoo.architectures.wide_resnet import WideResNet
    model = WideResNet(depth=34, widen_factor=10).cuda()
    model = torch.nn.DataParallel(model).cuda()
    model.load_state_dict(torch.load('WRN/models/cifar10/Wu2020Adversarial.pt'))
elif args.pre_trained == 'TRADES':
    from robustbench.model_zoo.architectures.wide_resnet import WideResNet
    model = WideResNet(depth=34, widen_factor=10, sub_block1=True).cuda()
    model.load_state_dict(torch.load('WRN/models/cifar10/Zhang2019Theoretically.pt'))
elif args.pre_trained == 'PGD':
    from robustbench.model_zoo.architectures.wide_resnet import WideResNet
    model = WideResNet(depth=34, widen_factor=10).cuda()
    model = torch.nn.DataParallel(model).cuda()
    model.load_state_dict(torch.load('WRN/models/cifar10/cifar10_wide10_linf_eps8.pth'))
else:
    assert 0 
...
if args.lr_schedule == 'cyclic':
    ...
    elif args.epochs==2 or args.epochs==6:
...
for epoch in range(args.epochs):
    ...
    for i, (X, y) in enumerate(train_loader):
        ...
        FAAL_DRO = DAW(train_batch_size =valid_num,
